...
class BookService extends ModelService
{
    ...
    public function create(array $attributes): Result
    {
        ...
        $this->bookLogService->create(["user_id" => $user->id, "book_id" => $record->id, "notes" => $attributes["notes"] ?? "-", "reason" => $attributes["reason"] ?? "-", "old_status" => "-", "new_status" => $attributes["status"]]);;
...
foreach ($updates as $update ){
    if ($update instanceof BookLog) {
        ...
        $users[$update->user_id]=$this->userService->find($update->user_id);
    }
}
